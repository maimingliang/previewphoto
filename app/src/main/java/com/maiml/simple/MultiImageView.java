...
public class MultiImageView extends LinearLayout {
	...
	// 照片的Url列表
	private List<PrePhotoInfo> imagesList;

	/** 长度 单位为Pixel **/
	private int pxOneMaxWandH;  // 单张图最大允许宽高
	private int pxMoreWandH = 0;// 多张图的宽高
	private int pxImagePadding = dp2px(getContext(), 3);// 图片间的间距
	...
	public void setList(List<PrePhotoInfo> lists) throws IllegalArgumentException {
		if(lists==null){
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		PrePhotoInfo photoInfo = imagesList.get(position);
		...
		if(isMultiImage){
            ...
            int expectW = photoInfo.getSmallWidth();
            int expectH = photoInfo.getSmallHeight();
